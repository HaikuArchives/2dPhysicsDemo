...
const rgb_color col1 	= 	{0,0,255};		// my own additions (Andreas Kaenner)
const rgb_color col2 	= 	{255,0,0};		// ""
...
real const Kws = r(60);			// Hooke's spring constant  --->  30
...
real const Kbd = r(3);			// damping constant
...
void simulation_world::Render( void )
{
	...
	for(Counter = 0;Counter < NumberOfBodies;Counter++)
	{
		rigid_body::configuration::bounding_box &Box =
			...
			BPolygon poly1; // my own additions (Andreas Kaenner)
		...
		//	Line(Box.aVertices[Start].X,Box.aVertices[Start].Y, Box.aVertices[End].X,Box.aVertices[End].Y);
			BPoint my_point(200+Box.aVertices[Start].X,200-Box.aVertices[Start].Y); // my own additions (Andreas Kaenner)
			poly1.AddPoints(&my_point,1); 											// ""
		}
		if(Counter)FillPoly(&poly1, col1);														// ""
		else FillPoly(&poly1, col2);
	}
